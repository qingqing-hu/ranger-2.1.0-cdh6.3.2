...
if [ "${HCOMPONENT_NAME}" = "hadoop" ] || [ "${HCOMPONENT_NAME}" = "hive" ]
then
  if [ ! -e "$HCOMPONENT_CONF_DIR/xasecure-audit.xml" ]
  then
    touch $HCOMPONENT_CONF_DIR/xasecure-audit.xml
    chown ${CFG_OWNER_INF} ${HCOMPONENT_CONF_DIR}/xasecure-audit.xml
    chmod a+rx ${HCOMPONENT_CONF_DIR}/xasecure-audit.xml
  fi
fi
